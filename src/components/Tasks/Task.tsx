...
export const Task: FC<TaskProps> = memo(({ id, text, completed, list, onRemove, onEdit, onComplete }) => {
  ...
  const handleEdit = useCallback(() => onEdit(list.id, { id, text }), [id, list, onEdit, text]);
  const handleRemove = useCallback(() => onRemove(list.id, id), [id, list, onRemove]);

  return (
    <div key={id} className="tasks__items-row">
      <div className="checkbox">
        <input name="task" onChange={handleChangeCheckbox} id={`task-${id}`} type="checkbox" checked={completed} />
      ...
      <div className="tasks__items-row-actions">
        ...
        <div role="presentation" onClick={handleRemove}>
